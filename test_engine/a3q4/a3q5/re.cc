...
namespace CS246E {
	...
	unique_ptr<RegExp> parseWord(const string & s) {
		const string str = unwrap(s); 
		if (s == "^") return make_unique<Begin>();
		else if (s == "$") return make_unique<End>();
		else if (s == ".") return make_unique<Dot>();
		else return make_unique<Word> (str);
		// parseWord is kind of versatile, but it saves time to call other functions
	} 
	...
	bool containsMatch(RegExp * re, const string & s) {
		...
		for (size_t i = 0; i < len; ++i) {
			for (size_t j = 1; j <= len - i; ++j) {
				if (re->matches(s2.substr(i, j))) return true;
			}
		}
		return false;
	}
